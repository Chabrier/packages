                                ...
                                const bool snap)
{
    ...
    if (dim == 1) { // history is not managed by conds for vectors
        if (val) {
            int lastFeed = 0;
            if (historySize > 0) {
                vv::Tuple* history = vv::Tuple::create();
                if (val->isDouble()) {
                    history->add(val->toDouble().value());
                    lastFeed = 1;
                } else {
                    uint j = 0;
                    while (lastFeed <= historySize &&
                           j < val->toTuple().size()) {
                        history->add(val->toTuple()[j]);
                        lastFeed++;
                        j++;
                    }
                }
                        ...
                        const bool snap)
{
    ...
    if (dim == 1) {
        if (val) {
            int lastFeed = 0;
            if (historySize > 0) {
                vv::Tuple* init = vv::Tuple::create();
                init->add(val->toTuple()[0]);
                lastFeed = 1;
                while (lastFeed <= historySize) {
                    init->add(0.0);
                    lastFeed++;
        ...
        if (val) {
            ...
            vv::Tuple* init = vv::Tuple::create();
            if (val->isDouble()) {
                init->add(val->toDouble().value());
                lastFeed = 1;
            } else {
                uint j = 0;
                while (lastFeed < dim &&
                       j < val->toTuple().size()) {
                    init->add(val->toTuple()[j]);
                    lastFeed++;
                    j++;
                }
            }
